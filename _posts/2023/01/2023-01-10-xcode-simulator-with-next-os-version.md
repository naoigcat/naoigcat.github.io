...
そのため旧バージョンのXcodeを使いながら新しいOSバージョンの**実機**で動作確認を行いたい場合がある。（シミュレーター用ランタイムは `DeviceSupport` ではなく `.simruntime` / CoreSimulator 側で別管理される。）

## 新しいOSバージョンの実機向けにデバッグ実行可能にする
...
上記手順で新しいOSバージョンの**実機**に対して、旧バージョンのXcodeからデバッグ実行できるようになる。 `DeviceSupport` は実機デバッグ用のシンボル／サポートファイルであり、シミュレーターの起動可否とは別問題である。

...
XcodeではCPUアーキテクチャごとにシミュレーター向けと実機向けのビルドを作成するが、外部ライブラリのarm64アーキテクチャが実機向けにしか対応していないため、このエラーが発生する。

## ライブラリ側で対応する

根本的な対応方法はライブラリを.xcframeworkに対応させてarm64アーキテクチャのシミュレーター向けビルドを含めることである。

以前のライブラリ形式では1つのCPUアーキテクチャにつき1種類のビルドしか含められないため、arm64アーキテクチャの実機向けビルドとシミュレーター向けビルドを共存させることができない。

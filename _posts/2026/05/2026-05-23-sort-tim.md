...
ティムソート (`timsort`) は、すでに整列している連続区間（ラン）を見つけ、短いランを挿入ソートで伸ばしながら、マージソートの枠組みでマージする。
...
4.  **マージの抑制**: プッシュのたびに、スタック上のラン長のバランスを見て隣接ランをマージする。最後に残ったランをすべてマージして全体を整列する。
...
マージ自体はマージソートと同様に 2 本の昇順列の先頭を比較しながら確定させる。実装ではガロップ（一方の列が連続して勝つときにまとめて取り込む）などの最適化も入るが、デモでは基本的な二分マージまでを追えるようにしている。

整列済みに近い入力では $$O(n)$$ に近づく適応型ソートで、最悪でも $$O(n \log n)$$ を保ち、安定ソートである。
...
[マージソート](/2026/05/03/sort-merge.html)と同様に $$O(n \log n)$$ の安定整列だが、すでに昇順の連続区間（ラン）を検出して伸ばし、小さなランは挿入ソートで仕上げるため、整列済み入力では $$O(n)$$ に近づく。クイックソートのように最悪計算量 $$O(n^2)$$ に落ちにくい点も強みである。
...
|-----------:|----------------:|----------------:|----------------:|----------------:|
|        256 |        0.000005 |        0.000049 |               2 |               2 |
|        512 |        0.000012 |        0.000061 |               4 |               4 |
|       1024 |        0.000026 |        0.000084 |               8 |               8 |
|       2048 |        0.000057 |        0.000108 |              16 |              16 |
|       4096 |        0.000124 |        0.000209 |              32 |              32 |
|       8192 |        0.000275 |        0.000464 |              64 |              64 |
|      16384 |        0.000596 |        0.000943 |             128 |             128 |
|      32768 |        0.001267 |        0.002180 |             256 |             256 |
|      65536 |        0.002756 |        0.004689 |             512 |             512 |
|     131072 |        0.005933 |        0.008475 |            1024 |            1024 |
|     262144 |        0.012649 |        0.017763 |            2048 |            2048 |

...
[マージソート](/2026/05/03/sort-merge.html)と同様に `O(n log n)` の安定整列だが、すでに昇順の連続区間（ラン）を検出して伸ばし、小さなランは挿入ソートで仕上げるため、整列済み入力では `O(n)` に近づく。クイックソートのように最悪計算量 `O(n²)` に落ちにくい点も強みである。

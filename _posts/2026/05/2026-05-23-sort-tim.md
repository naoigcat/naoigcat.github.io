...
整列済みに近い入力では `O(n)` に近づく適応型ソートで、安定ソートである。
...
[マージソート](/2026/05/03/sort-merge.html)と同様に `O(n log n)` の安定整列だが、既存の昇順ランを検出して伸ばし、小さなランは挿入ソートで仕上げる。

[クイックソート](/2026/05/02/sort-quick.html)のように最悪計算量 `O(n²)` に落ちにくく、整列済み入力では `O(n)` に近い。

...
トーナメントソート (`tournament sort`) は、配列の各要素を葉とする完全二分木を用意し、隣接する葉同士の比較で「勝者」（昇順なら小さい方のインデックス）を親へ伝播させるトーナメント木を構築する比較ソートである。
...
木用の補助配列に `O(n)` の追加領域が必要で、同一キーの相対順序を保たない不安定なソートである。

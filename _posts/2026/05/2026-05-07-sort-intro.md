...
クイックソート単体は入力次第でピボット選びが偏り、再帰深度が `O(n)` になり最悪計算量 `O(n²)` になり得る。
...
クイックソートの最悪計算量 `O(n²)` をヒープソートへの切り替えで避け、最悪計算量 `O(n log n)` を保つが、一般に不安定である。
...
[クイックソート](/2026/05/02/sort-quick.html)に深さ監視と[ヒープソート](/2026/05/04/sort-heap.html)への切り替え、[挿入ソート](/2026/05/05/sort-insertion.html)による小区間仕上げを加えたもので、クイックソート単体の最悪計算量 `O(n²)` を避けつつ平均性能を狙っている。

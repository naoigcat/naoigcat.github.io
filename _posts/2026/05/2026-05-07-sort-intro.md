...
クイックソート単体は入力次第でピボット選びが偏り、再帰深度が $$O(n)$$ に達して最悪計算量 $$O(n^2)$$ になり得る。

そこでイントロソートは許容する再帰の深さに上限（多くの実装で $$2 \cdot \lfloor \log_2 n \rfloor$$ 前後）を設け、それを超えそうな区間だけヒープソートにフォールバックする。比較ソートとしての下界 $$\Omega(n \log n)$$ に張り付いたまま、最悪ケースを回避できる。
  ...
  function generateSteps(initial) {
    ...
    function intro(lo, hi, depth) {
      ...
      if (depth === 0) {
        steps.push({
          kind: 'phase',
          text: '深さ上限に達したため、この範囲はヒープソートへ切り替え（最悪計算量 $$O(n \log n)$$ を担保）',
          arr: a.slice(),
        });
        heapsortRange(lo, hi);
        return;
...
[ロムート分割型クイックソート](/2026/05/02/sort-quick-lomuto.html)に深さ監視と[ヒープソート](/2026/05/04/sort-heap.html)への切り替え、[挿入ソート](/2026/05/05/sort-insertion.html)による小区間仕上げを加えたものである。

クイックソート単体の最悪計算量 $$O(n^2)$$ を避けつつ、平均性能を維持しようとする設計である。

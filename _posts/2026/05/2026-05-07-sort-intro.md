...
クイックソート単体は入力次第でピボット選びが偏り、再帰深度が `O(n)` になり最悪 `O(n²)` になり得る。イントロソートは許容する再帰の深さに上限（多くの実装で `2·⌊log₂ n⌋` 前後）を設け、それを超えそうな区間だけヒープソートにフォールバックすることで、比較ソートとしての下界 `Ω(n log n)` に張り付いたまま、最悪ケースを回避する。

...
サンプルソート (`sample sort`) は、入力から少数の標本を取り出して整列し、その値を分割点（スプリッター）として全体をバケットに分け、各バケットを整列して連結する。

並列計算向けに設計された手法として知られ、プロセッサ数 `p` に対して `p−1` 個のスプリッターを選ぶ構成が典型である。
...
1.  **サンプリング**: 配列から `s` 個の要素を（等間隔に）選び、サンプル集合を得る。文献では $$s \approx \sqrt{n}$$ や `s = p−1` などが用いられる。
2.  **サンプルの整列**: サンプルだけを整列し、昇順のスプリッター列 $$t_0 \le t_1 \le \ldots$$ を得る。
3.  **仕分け**: 各要素 `x` を、$$t_0, t_1, \ldots$$ と比較して属するバケット番号を決める（例: $$x \le t_0$$ ならバケット 0、$$t_0 < x \le t_1$$ ならバケット 1、…）。
...
逐次では標本の整列とバケットへの仕分けを合わせておおむね $$O(n \log n)$$、プロセッサ p 台では $$O(n/p)$$ 程度の並列時間を狙える。バケット境界での分割は交換を伴うため一般に不安定である。
...
外部ソートや分散整列の文脈では I/O 効率のよい分割としても使われる。サンプル数や再帰の打ち切り、等値要素の扱いが性能を左右する。

## 類似アルゴリズムとの相違点

[ロムート分割型クイックソート](/2026/05/02/sort-quick-lomuto.html)が 1 つのピボットで 2 分割するのに対し、標本から複数スプリッターを求めて同時に多分割する。並列・外部整列向きである。[バケットソート](/2026/06/23/sort-bucket.html)は値域を等分する点が異なる。
...
|-----------:|-----------------:|-----------------:|---------------------:|---------------------:|
|        256 |         0.000010 |         0.000058 |                    4 |                    6 |
|        512 |         0.000021 |         0.000146 |                    7 |                   10 |
|       1024 |         0.000040 |         0.000242 |                   14 |                   19 |
|       2048 |         0.000079 |         0.000140 |                   27 |                   32 |
|       4096 |         0.000152 |         0.000427 |                   52 |                   62 |
|       8192 |         0.000331 |         0.002167 |                  101 |                  120 |
|      16384 |         0.000557 |         0.000948 |                  198 |                  217 |
|      32768 |         0.001094 |         0.001764 |                  389 |                  429 |
|      65536 |         0.002252 |         0.004152 |                  766 |                  819 |
|     131072 |         0.004685 |         0.008009 |                 1518 |                 1612 |
|     262144 |         0.009153 |         0.015627 |                 3012 |                 3175 |

<!-- sort-benchmark-result:end -->

{% include sort-benchmark.md algorithm="sample" %}

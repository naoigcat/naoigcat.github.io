...
ストランドソート (`strand sort`) は、未整列区間を左から走査し、先頭から続く単調非減少部分列（ストランド）だけを抜き取り、整列済み結果とマージすることを繰り返す。

「麻ひも・繊維束（strand）をほどく」イメージで、1 本ずつ取り出して束ね直していく。

1.  **初期化**: 整列済み結果 `R` は空、`P` に入力をそのまま置く。
2.  **ストランド抽出**: `P` を左から見て、直前にストランドへ入れた末尾の値以上の要素だけを順にストランドへ移す（末尾より小さい値は見送り、インデックスだけ進める）。1 回の走査でストランドは必ず 1 要素以上になる。
3.  **マージ**: `R` とストランドはどちらも昇順なので、マージソートと同様に先頭同士を比較しながら併合し、新しい `R` とする。`P` に残った要素はそのまま次のラウンドへ。
4.  **終了**: `P` が空になるまで手順 2〜3 を繰り返す。

最悪計算量 `O(n²)` になるが、マージを安定実装すれば安定ソートにできる。
...
[マージソート](/2026/05/03/sort-merge.html)と同様に昇順部分列を併合するが、入力から単調列を抜き取る段とマージを交互に行う。汎用ソートとしてはマージやティムに劣りやすい。
...
|-----------:|----------------:|----------------:|----------------:|----------------:|
|        256 |        0.000019 |        0.000065 |              89 |             104 |
|        512 |        0.000034 |        0.000139 |             115 |             132 |
|       1024 |        0.000067 |        0.000117 |             115 |             136 |
|       2048 |        0.000141 |        0.000286 |             158 |             200 |
|       4096 |        0.000341 |        0.000420 |             213 |             268 |
|       8192 |        0.000936 |        0.001248 |             291 |             416 |
|      16384 |        0.002864 |        0.003605 |             494 |             724 |
|      32768 |        0.007856 |        0.013934 |            1014 |            1492 |
|      65536 |        0.021828 |        0.028007 |            1940 |            2820 |
|     131072 |        0.068399 |        0.429992 |            3786 |            5440 |
|     262144 |        0.195783 |        0.551794 |            7424 |           10824 |

<!-- sort-benchmark-result:end -->

{% include sort-benchmark.md algorithm="strand" %}

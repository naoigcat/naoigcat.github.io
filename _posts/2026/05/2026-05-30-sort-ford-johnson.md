...
1.  **ペアリング**: 隣接要素を $$\lfloor n/2 \rfloor$$ 組のペアに分け、 `(小, 大)` の順に並べる。要素数が奇数なら最後の1要素は保留列に回す。
...
[挿入ソート](/2026/05/05/sort-insertion.html)の $$O(n^2)$$ 比較を、ペアリングと主系列への二分挿入で抑える。実装は複雑だが比較回数の最小化が目的である。

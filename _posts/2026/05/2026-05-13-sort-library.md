...
図書館ソート (`library sort`) は、整列済みデータを隙間（空きスロット）を挟みながら並べ、新しい要素を二分探索で挿入位置を決めて空きへ置く（足りなければ隣接交換でずらす）。
...
概念的には「棚に空きを残しておく」ことで挿入ソートで毎回長いシフトが続く状況を緩和しようとする発想である。

## 類似アルゴリズムとの相違点

図書館ソートは[挿入ソート](/2026/05/05/sort-insertion.html)の派生で、整列済みの値のあいだに空きマスを残して挿入し、位置の探索に二分探索を使う。ランダム入力では比較回数が `O(n log n)` に近づきやすい。

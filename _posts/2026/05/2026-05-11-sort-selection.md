...
選択ソート (`selection sort`) は、未整列の範囲から最小（または最大）の要素を1つ選び、先頭側の未確定位置と交換することを繰り返す比較ソートである。
...
最悪・平均・最良いずれも比較回数は `O(n²)` で、入力の並びで大きくは変わらない。交換回数は高々 `O(n)` と少ないのが特徴である。追加配列を使わなければ空間計算量は `O(1)` のインプレースソート。等しい値同士の順序を入れ替える実装になりやすく、一般に不安定なソートとして扱われる。

クイックソートやマージソートと比べると漸近的な効率は劣るが、実装が短く、動きの説明に向く。

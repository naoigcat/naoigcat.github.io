...
通常の二分ヒープでは、根が配列の一端にあり、最大値を取り出すたびに **根と末尾を交換して** ヒープを底側から縮めていく。この配置は「ヒープとしての整理」と「昇順に書き出す向き」が噛み合わず、**すでにソート済みの入力でも一度ヒープ状に混ぜ直す**ため、最良でも必ず Ω(n log n) 程度の仕事が残る。

スムースソートでは、二分木ではなく **レオナルド数** に基づく **レオナルド木** を積み重ねた **森** を維持する。
各木はヒープ条件を満たし、さらに **各木の根の値は左から右へ弱い順（非減少）** として並べる。
そうすると **常に右端が全体の最大** になり、末尾から確定した最大値を「取り出す」処理が **すでに整っている入力ではほとんど余計な比較を要しない**。

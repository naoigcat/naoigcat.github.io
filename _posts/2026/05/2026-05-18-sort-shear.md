...
二次元格子上の行・列整列を繰り返す並列向けソートで、要素数 N に対し $$\Theta(\sqrt{N} \log N)$$ ステップで収まる。行・列の内部ソートが不安定なら全体も崩れやすく、一般には不安定ソートとして扱われる。
...
[バブルソート](/2026/05/01/sort-bubble.html)や[ロムート分割型クイックソート](/2026/05/02/sort-quick-lomuto.html)は一次元配列を直接変更する。シアソートは二次元格子の行・列整列だけが許され、並列通信モデルが前提である。

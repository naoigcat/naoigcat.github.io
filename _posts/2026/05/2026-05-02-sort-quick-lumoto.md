...
分割（partition）に Nico Lomuto の方式を使う点が特徴である。右端をピボットに据え、走査ポインタ `j` と境界ポインタ `i` の 2 本だけで左右を切り分ける。境界の意味が読みやすく、学習用の実装や可視化に向いている。
...
Hoare の分割法と比べると、境界が一目で分かる反面、交換回数が増えやすく、末尾をそのままピボットにすると昇順・降順に近い入力で分割が偏りやすい。実用ではランダムピボットや三点中央値などのピボット選択と組み合わせ、偏りを起こしにくくすることが多い。

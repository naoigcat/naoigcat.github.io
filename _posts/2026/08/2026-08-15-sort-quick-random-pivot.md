...
[ロムート分割型クイックソート](/2026/05/02/sort-quick-lomuto.html)が右端固定だと昇順・降順に近い入力で最悪計算量 `O(n²)` になりやすいのに対し、乱択ピボットは入力の並び方に依存しにくく、期待計算量を `O(n log n)` に近づけやすい。分割そのものはロムート方式を使い、違いは「どの位置をピボットにするか」にある。

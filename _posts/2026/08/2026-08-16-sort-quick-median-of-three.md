...
[ロムート分割型クイックソート](/2026/05/02/sort-quick-lomuto.html)が右端固定だと昇順・降順に近い入力で最悪計算量 `O(n²)` になりやすいのに対し、三点中央値は整列済みやほぼ整列済みでも端の極端な値を避けやすい。分割そのものはロムート方式を使い、違いは「どの位置をピボットにするか」にある。

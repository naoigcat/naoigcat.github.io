...
整数を十進の各桁で処理する場合、桁数を `d`、基数を `r`（十進なら `r = 10`）とすると時間計算量は `O(d · (n + r))`、
補助配列を含む空間計算量は `O(n + r)` である。値域が `n` に対して多桁でも、桁数 `d` が `log_r(max)` 程度に抑えられるとき、
カウンティングソート単体より値域が広いキーにも適用しやすい。文字列や固定長キー、ビット列を基数 `256` などで処理する
MSD（Most Significant Digit）版もある。

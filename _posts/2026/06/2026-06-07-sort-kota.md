...
コタソート（`kota sort`）は、ボトムアップのマージソートの枠組みで、長さ $$O(\sqrt{n})$$ 程度のブロック単位でマージし、マージのあとにブロック選択ソートでブロックの順序を整える。

グレイルソートがマージ前にブロック選択を行い、ウィキソートがマージと同時にブロック選択を行うのに対し、コタソートはマージ後にブロック選択を行う。ブロックマージのロジックが最も単純で実装しやすく、移動回数とマージ比較が最適化されている。多方向ブロックマージへの一般化も可能である。

1.  **小さなラン整列**: 長さ 16 程度以下の区間は挿入ソートで整える。
2.  **ボトムアップマージ**: 隣接する整列済み部分列を段階的に倍化しながらマージする。
3.  **ブロックマージ**: 区間が十分大きくなると、長さ $$\sqrt{n}$$ 程度のブロックに分割し、外部バッファを使って隣接ブロックを順にマージする。
4.  **ブロック選択**: マージが終わったブロック列に対し、各ブロックの先頭要素をキーとして選択ソートを行い、ブロック全体の順序を決める。安定性のため、別のキーバッファでブロックの元の相対順序を追跡する。
5.  **キーの復元**: 収集した一意なキー列（約 $$3\sqrt{n}$$ 個）を挿入ソートし、全体へマージして整列を完了する。
...
コタソートはバッファレス変種を持たないため、配列全体を完結させるには別のブロックマージソート（グレイルソートやウィキソートなど）との組み合わせが必要になる。キー数は他のブロックマージ変種と比べて最も多く、約 $$3\sqrt{n}$$ 個を要する。外部バッファもブロック長（おおむね $$\sqrt{n}$$）ぶん要るため、補助記憶はおおむね $$O(\sqrt{n})$$ である。
...
window.DemoSort && DemoSort.boot('kota-sort-demo', function (root) {
  ...
  DemoSort.attachPlayback({
    ...
    initialCaption:
      'コタソートのデモ（マージ後にブロック選択；比較はオレンジ、確定書き込みは緑、ブロック選択は紫）',
...
最悪計算量 $$O(n \log n)$$ の安定ソートで、ブロックマージ系の中では実装が単純である。本番寄りの補助記憶はキーと外部バッファ合わせて $$O(\sqrt{n})$$ だが、計測コードは説明用にラン全体を退避するマージバッファを使うため最大 $$O(n)$$ になる。

## 類似アルゴリズムとの相違点

[グレイルソート](/2026/06/01/sort-grail.html)はマージ前、[ウィキソート](/2026/05/31/sort-wiki.html)はマージと同時にブロック選択する。コタはマージ後にブロック選択し、3 者のうちロジックが最も単純である。
...
|-----------:|----------------:|----------------:|----------------:|----------------:|
|        256 |        0.000007 |        0.000068 |               1 |               1 |
|        512 |        0.000015 |        0.000057 |               2 |               2 |
|       1024 |        0.000034 |        0.000075 |               4 |               4 |
|       2048 |        0.000074 |        0.000180 |               8 |               8 |
|       4096 |        0.000165 |        0.000285 |              16 |              16 |
|       8192 |        0.000346 |        0.000582 |              32 |              32 |
|      16384 |        0.000685 |        0.001314 |              64 |              64 |
|      32768 |        0.001429 |        0.002805 |             128 |             128 |
|      65536 |        0.003013 |        0.004925 |             256 |             256 |
|     131072 |        0.006461 |        0.009607 |             512 |             512 |
|     262144 |        0.013878 |        0.024927 |            1024 |            1024 |

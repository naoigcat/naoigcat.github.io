...
グレイルソート (`Grailsort`) は、Andrey Astrelin によるブロックマージソート（`block merge sort`）の実装である。

Huang と Langston の「定数追加空間での高速安定マージ・整列」に基づく発想を取り入れ、安定かつ最悪 `O(n log n)` で、追加記憶領域を `O(1)`（固定サイズの外部バッファを使う変種ではスタック上の 512 要素程度）に抑えた整列を目指す。

グレイルソートは配列先頭付近に一意なキーを集め、それを内部バッファおよびブロックタグとして使い回す点が特徴的である。
...
マージソートのように常に `O(n)` の補助配列を確保しない点、クイックソートのように最悪 `O(n²)` に落ちない点が、インプレースかつ安定な整列を求める場面での利点になる。
...
デモは、グレイルソートのキー収集とボトムアップ併合の流れを追いやすくするため、小さな配列で一意なキーの検出と、キャッシュに収まるレベルでのマージ（マージソートと同型の併合）までを可視化している。

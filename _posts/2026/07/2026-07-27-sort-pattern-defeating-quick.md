...
基本は[ロムート分割型クイックソート](/2026/05/02/sort-quick-lomuto.html)だが、次の仕掛けで「悪い入力パターン」を撃退する。
...
3.  **不均衡の検知**: 左右の部分配列が元サイズの 1/8 未満に偏ったら、候補位置を入れ替えてパターンを崩す。偏りが $$\lfloor \log_2 n \rfloor$$ 回続くと[ヒープソート](/2026/05/04/sort-heap.html)へフォールバックする。

...
1.  **値域とクラス数**: 配列の最小値 `min`・最大値 `max` を求め、クラス数 `m` を $$\lceil \sqrt{2 n \log_2 n} \rceil$$ 程度（実装によっては `0.45 n` 付近）に設定する。
2.  **出現数の集計**: 各要素 `x` について $$k = \lfloor (m - 1)(x - min) / (max - min) \rfloor$$ でクラス番号を求め、クラスごとの要素数を数える。
...
分布が一様であれば $$O(n)$$ に近づくが、多くの要素が同一クラスへ集中するとクラス内の挿入ソートが重なり $$O(n^2)$$ になりうる。循環交換による配置は等値の相対順を崩すため一般に不安定である。

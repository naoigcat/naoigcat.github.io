...
フラッシュソート (`flash sort`) はキーの最小値・最大値から値域を `m` 個のクラス（区分）に等分し、各要素を対応クラスへ仕分けたあと、クラス内を挿入ソートで仕上げる分布型ソートである。
...
分布が一様で各クラスのサイズが定数に抑えられるとき、仕分けと配置は `O(n)`、クラス内の挿入ソートも合計 `O(n)` となり、全体は `O(n)` 程度と見積もれる。
最悪はすべての要素が同一クラスに入り、挿入ソート 1 回分の `O(n²)` になる。追加メモリはクラス数 `m` に比例するカウント・境界配列の `O(m)` が典型で、`m` は `O(√(n log n))` 程度に抑えられる。

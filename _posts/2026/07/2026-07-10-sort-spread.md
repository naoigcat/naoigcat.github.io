...
ハイブリッド型の分布ソートである。Boost.Sort ライブラリにも実装があり、整数・浮動小数・文字列向けに最適化された派生が含まれる。
...
[フラッシュソート](/2026/07/09/sort-flash.html)も値域をビンに分けるが、スプレッドはビン数を `n/c` 付近にし、要素数に応じて再帰か `O(n log n)` 比較ソートへフォールバックする。[プロックスマップソート](/2026/06/30/sort-proxmap.html)はオンライン配置が異なる。

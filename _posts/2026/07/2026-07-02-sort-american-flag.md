...
アメリカ国旗ソート (`american flag sort`) は、キーを固定幅の記号列（典型例は 1 バイト単位）としてみなし、現在位置の記号値ごとにカウンティングしてバケット境界を決め、インプレースの交換で各要素を対応バケットへ集める処理を再帰的に実行する。
...
2.  **出現回数の集計**: 現在位置の記号 $$0,\ldots,\sigma-1$$（バイトなら $$\sigma = 256$$）について各出現数を数える。
...
整数キーを `usize` として整列するときは、最上位バイトから下位バイトへと `digit(x, byte)` を取り、記号集合サイズ $$\sigma = 256$$ として上記を適用するのが典型である（下の計測コードもこの方式）。

記号幅 w なら $$O(w \cdot (n + \sigma))$$ となり、カウンティング配列 $$O(\sigma)$$ だけのインプレース志向の実装が多い。不安定である。

以下のデモでは視認性のため十進の各桁（$$\sigma = 10$$）を**上位桁から**同じ手順で示す。バイト列版と違いは記号の取り方だけで、バケット形成と再帰の流れは同じである。
...
window.DemoSort && DemoSort.boot('american-flag-sort-demo', function (root) {
  ...
  DemoSort.attachPlayback({
    ...
    initialCaption:
      'アメリカ国旗ソートのデモ（十進・最上位桁優先、棒上に値、バケット境界は背景色）',
...
[基数ソート](/2026/06/21/sort-radix.html)の最上位桁優先版に近いが、各桁（記号）ごとにインプレースでバケット境界へ集める。カウンティング配列 $$O(\sigma)$$ だけで済ませやすい。
...
|-----------:|-----------------:|-----------------:|---------------------:|---------------------:|
|        256 |         0.000019 |         0.000081 |                    0 |                    0 |
|        512 |         0.000028 |         0.000112 |                    0 |                    0 |
|       1024 |         0.000040 |         0.000116 |                    0 |                    0 |
|       2048 |         0.000066 |         0.000130 |                    0 |                    0 |
|       4096 |         0.000116 |         0.000506 |                    0 |                    0 |
|       8192 |         0.000209 |         0.000355 |                    0 |                    0 |
|      16384 |         0.000368 |         0.000663 |                    0 |                    0 |
|      32768 |         0.000713 |         0.001608 |                    0 |                    0 |
|      65536 |         0.001427 |         0.009719 |                    0 |                    0 |
|     131072 |         0.003634 |         0.008644 |                    0 |                    0 |
|     262144 |         0.006506 |         0.011646 |                    0 |                    0 |
